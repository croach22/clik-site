...
// Skeleton-UI language (muted placeholder bars + real chrome + a few accent
// chips) rather than literal data — the surface reads as "our product" without
// pretending to be a screenshot. Product screenshots can replace any of these
// later by swapping the component out in FeaturesBento.
...
function Window({
  ...
  return (
    <div
      ...
      style={{ borderColor: C(0.1), background: PANEL }}
    >
      <div className="flex items-center gap-1.5 border-b px-3 py-2" style={{ borderColor: C(0.08) }}>
        <span className="block h-1.5 w-1.5 rounded-full" style={{ background: C(0.18) }} />
        <span className="block h-1.5 w-1.5 rounded-full" style={{ background: C(0.18) }} />
        <span className="block h-1.5 w-1.5 rounded-full" style={{ background: C(0.18) }} />
        <span className="ml-2 font-mono" style={{ fontSize: 9, color: C(0.4) }}>
          {label}
        </span>
      </div>
      {children}
...
// Shared entrance: children assemble in sequence when scrolled into view
function useReveal() {
...
// ── 1. Brand & org memory ────────────────────────────────────
// A memory/settings screen: saved rules the engine applies to every project.
const MEMORY_ITEMS = [
    ...
    return (
      <div className="flex h-full w-full items-center justify-center gap-1">
        <span
          ...
          style={{ width: 10, height: '58%', background: `${accent}`, opacity: 0.9 }}
        />
          ...
          <div className="grid grid-cols-2 gap-1.5">
            {MEMORY_ITEMS.map((item, i) => (
              ...
              >
                ...
                <div
                  className="rounded"
                  style={{ height: 34, background: INSET, border: `1px solid ${C(0.06)}` }}
                >
...
// ── 2. Planning agent ────────────────────────────────────────
// High-level: how the agent reasons over a batch, abstracted from the UI.
const PLAN_STEPS = [
...
export function PlanningVisual() {
  ...
  return (
    <div ref={ref}>
      <Window label="planning agent">
        <div className="relative p-4">
          {/* connector rail */}
          <span
            className="absolute left-[26px] top-8 bottom-8 w-px"
            ...
            {PLAN_STEPS.map((step, i) => (
              ...
              >
                <span
                  ...
                  style={{ background: PANEL, border: `1.5px solid ${step.accent}` }}
                >
            ...
            <div className="grid grid-cols-4 gap-1.5">
              {[ROYCE, SALMON, SAGE, LAVENDER].map((a, i) => (
                <div
                  ...
                  style={{ background: `${a}12`, border: `1px solid ${a}33` }}
                >
...
// ── 3. Edit / build agent ────────────────────────────────────
// Timeline surface: dead air removed, B-roll matched by meaning.
const TALK_SEGMENTS = [
  { w: 16, gap: false },
  { w: 5, gap: true },
  { w: 22, gap: false },
    ...
    <div ref={ref}>
      <Window label="build agent">
        <div className="space-y-3 p-4">
          ...
          {/* dialogue track */}
          <motion.div {...rise(1)} animate={inView ? rise(1).animate : rise(1).initial}>
          ...
          {/* b-roll track */}
          <motion.div {...rise(2)} animate={inView ? rise(2).animate : rise(2).initial}>
            <div className="mb-1.5 flex items-center justify-between">
              <span className="font-mono uppercase" style={{ fontSize: 8, letterSpacing: '0.12em', color: C(0.4) }}>
                B-roll · matched by meaning
              </span>
            ...
            <div className="relative" style={{ height: 26 }}>
              <span
                ...
                style={{ background: INSET, border: `1px solid ${C(0.06)}` }}
              />
              {BROLL.map((b) => (
                <span
                  key={b.label}
      ...
      <Window label="projects">
        <div className="p-3">
                  ...
                  {col.cards.map((_, i) => (
                    ...
                    >
                      ...
                      <div className="flex items-center gap-1">
                        <span
                          className="block rounded-sm"
                          style={{ width: 12, height: 8, background: `${col.accent}44` }}
                        />
                        <span
                          className="block rounded-sm"
                          style={{ width: 12, height: 8, background: `${col.accent}26` }}
                        />
          ...
          <motion.div
            ...
            className="mt-2.5 flex items-center justify-between rounded-md border px-2.5 py-1.5"
            style={{ borderColor: C(0.09), background: INSET }}
          >
            ...
            <div className="flex -space-x-1">
              {[0.16, 0.12, 0.09].map((o, i) => (
                <span
                  ...
                  style={{ background: C(o), border: `1px solid ${PANEL}` }}
                />
              ))}
            </div>
          </motion.div>
